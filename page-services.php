<?php get_header(); ?>   
  <main class="page-main container">
  	<aside class="page-aside col-sm-4">
  		<section class="category">  		
  			<h1 class="category__header hidden-xs">Услуги</h1>
  			<ul class="hidden-xs" role="tablist">
					<li role="presentation" class="active"><a href="#1" aria-controls="1" role="tab" data-toggle="tab">Охрана стационарных объектов</a></li>
					<li role="presentation"><a href="#2" aria-controls="2" role="tab" data-toggle="tab">Обеспечение порядка в местах проведения массовых мероприятий</a></li>
<!--					<li role="presentation"><a href="#3" aria-controls="3" role="tab" data-toggle="tab">Личная охрана</a></li>					-->
<!--					<li role="presentation"><a href="#4" aria-controls="5" role="tab" data-toggle="tab">Пультовая охрана объектов</a></li>-->
					<li role="presentation"><a href="#5" aria-controls="7" role="tab" data-toggle="tab">Технические системы безопасности</a></li>
					<li role="presentation"><a href="#6" aria-controls="9" role="tab" data-toggle="tab">Юридические услуги</a></li>					
				</ul>
        <div class="mobile-menu visible-xs">
          <button class="mobile-menu__button">Услуги <span class="caret"></span></button>           
          <ul class="mobile-menu__list">
            <li role="presentation" class="active"><a href="#1" aria-controls="1" role="tab" data-toggle="tab">Охрана стационарных объектов</a></li>
            <li role="presentation"><a href="#2" aria-controls="2" role="tab" data-toggle="tab">Обеспечение порядка в местах проведения массовых мероприятий</a></li>
<!--            <li role="presentation"><a href="#3" aria-controls="3" role="tab" data-toggle="tab">Личная охрана</a></li>					-->
<!--            <li role="presentation"><a href="#4" aria-controls="5" role="tab" data-toggle="tab">Пультовая охрана объектов</a></li>-->
            <li role="presentation"><a href="#5" aria-controls="7" role="tab" data-toggle="tab">Технические системы безопасности</a></li>
            <li role="presentation"><a href="#6" aria-controls="9" role="tab" data-toggle="tab">Юридические услуги</a></li>	
          </ul>  
        </div>       
  		</section>
  		<section class="popular hidden-xs">
  			<div class="popular__wrapper">
  				<h2 class="popular__header"><span>Самое популярное</span></h2>
  				<div class="popular__container">
  					<div class="popular__item row">
							<a href="http://regionchop.ru/licenses">Лицензии</a>
							<p>Перечень лицензий, имеющихся у ЧОО «Тулрегионохрана».</p>
  					</div>
  					<div class="popular__item row">
							<a href="http://regionchop.ru/customers">Отзывы</a>
							<p>Наш профессионализм лучше всего подтверждает перечень наших заказчиков, доверяющих нам решение серьёзных охранных задач.</p>
  					</div>
  					<div class="popular__item row">
							<a href="http://regionchop.ru/prices">Цены</a>
							<p>Ценовая политика нашей организации. Работаем на взаимовыгодных условиях.</p>
  					</div>
  				</div>
  			</div>
  		</section>
  	</aside>
  	<article class="services col-sm-8">  	
  		<div class="services__wrapper">
  			<div class="services__container">
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane fade in active" id="1">
						  <h2 class="services__header"><span>Охрана стационарных объектов</span></h2>
						  <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/img/services/1.jpg" alt="">
						  <p>Охрана стационарных объектов включает в себя ряд мероприятий, обеспечивающих безопасное и непрерывное функционирование работы предприятия, любого назначения. При становлении организации жизнеспособности бизнеса, важно понимать, какие риски и какие опасности могут произойти. Немаловажную роль играет местоположение объекта, поэтому при приеме и осмотре объекта, наша компания рассматривает все варианты чрезвычайных ситуаций. Для пресечения подобных происшествий, ЧОО «Тулрегионохрана» использует ряд проверенных опытом мероприятий, полностью исключающих незаконные действия, как со стороны внутреннего персонала, так и сторонних лиц.</p> 
							<p>ЧОО «Тулрегионохрана» предлагает качественный спектр услуг по обеспечению безопасности стационарных объектов. Основная задача данного мероприятия – предотвращения преступных действий на объекте, путем выявления их на ранней стадии.</p>
						  <!--<div class="table-responsive">
								<table class="prices__table table">
									<thead>
										<tr>
											<th rowspan="2">№ п/п</th>
											<th rowspan="2">Наименование услуги</th>
											<th rowspan="2">Ед. изм.</th>
											<th colspan="2">Цена, руб.</th>
										</tr>
										<tr>
											<th>до 12 ч</th>
											<th>до 24 ч</th>
										</tr>
									</thead>
									<tbody>
										<tr>
											<td>1</td>
											<td>Стационарные объекты I категории</td>
											<td>чел/ч</td>
											<td>215</td>
											<td>160</td>
										</tr>
										<tr>
											<td>2</td>
											<td>Стационарные объекты II категории</td>
											<td>чел/ч</td>
											<td>225</td>
											<td>180</td>
										</tr>
										<tr>
											<td>3</td>
											<td>Стационарные объекты III категории</td>
											<td>чел/ч</td>
											<td>250</td>
											<td>220</td>
										</tr>
									</tbody>
								</table>
							</div>-->
						</div>
						<div role="tabpanel" class="tab-pane fade" id="2">
							<h2 class="services__header"><span>Обеспечение порядка в местах проведения массовых мероприятий</span></h2>
							<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/img/services/2.jpg" alt="">
							<p>Любое культурно-массовое мероприятие нуждается в организации безопасности, в связи с чем, ЧОО «Тулрегионохрана» гарантирует успешное проведение любого типа события, вне зависимости от количества посетителей.</p>
							<p>В основу организации охранной кампании подлежат обязательной проработке такие пункты, как:</p>
							<ul>
								<li>Персональный подход к разработке концепции безопасности мероприятия;</li>
								<li>Оборудование мероприятия всеми необходимыми средствами безопасности;</li>
								<li>Согласование действий охраны с организаторами и с личной охраной участников;</li>
								<li>Обеспечение пропускного режима на мероприятии;</li>
								<li>Бдительность при пропуске посетителей и персонала, во избежание проноса запрещенных предметов;</li>
								<li>Выявление и пресечение возможных конфликтов;</li>
								<li>Патрулирование мероприятия и прилегающей территории, в течении всего времени проведения;</li>
								<li>Защита жизни и здоровья посетителей от противоправных посягательств;</li>
							</ul>
							<p>Специалисты ЧОО «Тулрегионохрана» гарантируют спокойное проведение вашего мероприятия.</p>							
							<div class="table-responsive">
								<table class="prices__table table">
									<thead>
										<tr>
											<th rowspan="2">№ п/п</th>
											<th rowspan="2">Наименование услуги</th>
											<th rowspan="2">Ед. изм.</th>
											<th colspan="2">Цена, руб.</th>
										</tr>
										<tr>
											<th>до 12 ч</th>
											<th>до 24 ч</th>
										</tr>
									</thead>
									<tbody>								
										<tr>
											<td>1</td>
											<td>Охрана массовых мероприятий до 50 человек на постоянной основе</td>
											<td>чел/ч</td>
											<td>250</td>
											<td>280</td>
										</tr>
										<tr>
											<td>2</td>
											<td>Охрана массовых мероприятий более 50 человек на постоянной основе</td>
											<td>чел/ч</td>
											<td>280</td>
											<td>310</td>
										</tr>
										<tr>
											<td>3</td>
											<td>Охрана массовых мероприятий до 50 человек на разовой основе</td>
											<td>чел/ч</td>
											<td>280</td>
											<td>310</td>
										</tr>
										<tr>
											<td>4</td>
											<td>Охрана массовых мероприятий более 50 человек на разовой основе</td>
											<td>чел/ч</td>
											<td>310</td>
											<td>340</td>
										</tr>
									</tbody>
								</table>
							</div>
						</div>
<!--
						<div role="tabpanel" class="tab-pane fade" id="3">
							<h2 class="services__header">Личная охрана</h2>
							
						</div>
-->
<!--
						<div role="tabpanel" class="tab-pane fade" id="4">
							<h2 class="services__header"><span>Пультовая охрана объектов</span></h2>
							<img src="img/services/4.png" alt="">
						</div>
-->
						<div role="tabpanel" class="tab-pane fade" id="5">
							<h2 class="services__header"><span>Технические системы безопасности</span></h2>
							<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/img/services/5.png" alt="">
							<p>ЧОО «Тулрегионохрана» предлагает своим клиентам охрану объектов с использованием самых современных технических систем безопасности.</p>
							<p>В основе этого решения лежат технологии от ведущих российских и зарубежных производителей, чья продукция прекрасно зарекомендовала себя в сфере защиты имущества. Благодаря функциональности современных устройств, каждая система оптимально соотносит финансовые возможности клиента и перечень поставленных задач.</p>
							<p>На базе ЧОО «Тулрегионохрана» работает собственный отдел  инженерно-технических средств охраны (ИТСО), специалисты которого прошли соответствующее обучение и по праву считаются одними из лучших в отрасли. Сотрудники отдела ИТСО выполняют работы с техническими системами безопасности на объектах любой сложности: от квартир и частных домов до ритейла, банковского сектора и производства.</p>
						</div>
						<div role="tabpanel" class="tab-pane fade" id="6">
							<h2 class="services__header"><span>Юридические услуги</span></h2>
							<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/img/services/6.jpg" alt="">
							<p>ЧОО «Тулрегионохрана» готова оказать содействие при необходимости решения любого вида юридических вопросов. Мы готовы оказать услуги:</p>
							<ul>
								<li>Юридические консультации;</li>
								<li>Представительство интересов клиента в суде;</li>
								<li>Разрешение спорных ситуаций, носящих внесудебный претензионный характер;</li>
								<li>Разработка и юридическая экспертиза документации;</li>
								<li>Анализ споров и судебных разбирательств, обеспечительных мер по исполнительному производству, при производстве по арбитражным, уголовным, гражданским и административным делам;</li>
								<li>Юридическое сопровождение процедуры взыскания задолженностей;</li>
								<li>Правовая охрана и защита товарных знаков.</li>
							</ul>
						</div>
					</div>
				</div>
  			</div>
  		</div>
  	</article>	
  </main>
 <?php get_footer(); ?> 